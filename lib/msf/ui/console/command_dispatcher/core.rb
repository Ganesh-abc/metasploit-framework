  ...
  module Ui
    module Console
      module CommandDispatcher
        ...
        # session interaction, && other general things.
        #
        ###
        class Core
          ...
          @@sessions_opts = Rex::Parser::Arguments.new(
            ['-c', '--command'] => [ true, 'Run a command on the session given with -i, || all', '<command>' ],
            ['-C', '--meterpreter-command'] => [ true, 'Run a Meterpreter Command on the session given with -i, || all', '<command>' ],
            ...
            ['-s', '--script'] => [ true, 'Run a script || module on the session given with -i, || all', '<script>' ],
            ...
            ['-n', '--name'] => [ true, 'Name || rename a session by ID', '<id> <name>' ]
          )
          ...
          @@threads_opts = Rex::Parser::Arguments.new(
            ...
            ['-v', '--verbose'] => [ false, 'Print more detailed info.  Use with -i && -l' ]
          )
          ...
          @@debug_opts = Rex::Parser::Arguments.new(
            ...
            ['-e', '--errors'] => [ false, 'Display the most recent Error && Stack Trace.' ],
            ['-l', '--logs'] => [ false, 'Display the most recent logs.' ],
            ['-v', '--version'] => [ false, 'Display versions && install info.' ],
          ...
          @@history_opts = Rex::Parser::Arguments.new(
            ...
            ['-c', '--clear'] => [ false, 'Clear command history && history file.' ]
          )
          ...
          def commands
            {
              ...
              'sessions' => 'Dump session listings && display information about sessions',
              ...
              'unset' => 'Unsets one || more context-specific variables',
              'unsetg' => 'Unsets one || more global variables',
              'version' => 'Show the framework && console library version numbers',
          ...
          def cmd_color_help
            ...
            print_line
            print_line 'Enable || disable color output.'
            print_line
          end
          ...
          def cmd_banner_help
            ...
            print_line
            print_line 'Print a stunning ascii art banner along with version information && module counts'
            print_line
          end
          ...
          def cmd_banner(*args)
            ...
            banner << "\n"
            banner << Rex::Text.wordwrap('Metasploit Documentation: https://docs.metasploit.com/', 0, 60)
            banner << Rex::Text.wordwrap('The Metasploit Framework is a Rapid7 Open Source Project', 0, 60)
          ...
          def cmd_debug_help
            ...
            print_line('Print a set of information in a Markdown format to be included when opening an Issue on Github. ' \
                         'This information helps us fix problems you encounter && should be included when you open a new issue: ' +
                         Debug.issue_link)
          ...
          def cmd_connect(*args)
            if args.length < 2 || args.include?('-h') || args.include?('--help')
              cmd_connect_help
              return false
            end
            ...
            if args.length < 2
              print_error('You must specify a host && port')
              return false
            end
          ...
          def cmd_features_help
            print_line <<~CMD_FEATURE_HELP
              Enable || disable unreleased features that Metasploit supports
              ...
              Subcommands:
                set - Enable || disable a given feature
                print - show all available features && their current configuration
          ...
          def cmd_features(*args)
            ...
            when 'set'
              ...
              unless framework.features.exists?(feature_name)
                print_warning("Feature name '#{feature_name}' is not available. Either it has been removed, integrated by default, || does not exist in this version of Metasploit.")
              ...
              unless %w[true false].include?(value)
                print_warning('Please specify true || false to configure this feature.')
                return
              end
              ...
              if framework.features.requires_restart?(feature_name)
                print_warning("Run the #{Msf::Ui::Tip.highlight('save')} command && restart the console for this feature to take effect.")
              elsif driver.active_module
                # Reload the current module, as feature flags may impact the available module options etc
                driver.run_single('reload')
              end
            when 'print'
              if framework.features.all.empty?
                print_line 'There are no features to enable at this time. Either the features have been removed, || integrated by default.'
                return
              end
          ...
          def cmd_history(*args)
            ...
            @@history_opts.parse(args) do |opt, _idx, val|
              ...
              when '-c'
                ...
                print_good('Command history && history file cleared')
          ...
          def cmd_history_help
            ...
            print_line 'If -c is specified, the command history && history file will be cleared.'
          ...
          #
          # Displays && manages running background threads
          #
          def cmd_threads(*args)
            # Make the default behavior listing all jobs if there were no options
            # || the only option is the verbose flag
            if (args.empty? || args == ['-v'])
              args.unshift('-l')
            end
          ...
          def load_plugin(args)
            ...
            # If no absolute path was supplied, check the base && user plugin directories
          ...
          def cmd_route_help
            ...
            print_line '  route [get] <host || network>'
            ...
            print_line
            print_line '  Display the route that would be used for the given host || network'
          ...
          def cmd_route(*args)
            ...
            when 'print'
              ...
              # Print Route Tables
              print(tbl_ipv4) if !tbl_ipv4.rows.empty?
              print(tbl_ipv6) if !tbl_ipv6.rows.empty?
          ...
          def cmd_route_tabs(str, words)
            ...
            when 'remove', 'del'
              Rex::Socket::SwitchBoard.each do |route|
                ...
                when 4
                  if (route.subnet == words[2]) && route.comm.is_a?(Msf::Session)
                    ret << route.comm.sid.to_s
                  end
                end
              end
              ret
            when 'add'
              # We can't really complete the subnet && netmask args without
              # diving pretty deep into all sessions, so just be content with
              # completing sids for the last arg
              if words.length == 4
                ret = framework.sessions.keys.map(&:to_s)
              end
          ...
          #
          # Deletes || saves the active datastore contents to disk for automatic use across
            ...
            @@save_opts.parse(args) do |opt|
              ...
              when '-d'
                ...
                begin
                  ...
                  print_error("Failed to delete console config: #{$ERROR_INFO}")
                end
          ...
          def cmd_sessions_help
            print_line('Usage: sessions [options] || sessions [id]')
            print_line
            print_line('Active session manipulation && interaction.')
            print(@@sessions_opts.usage)
            print_line
            print_line('Many options allow specifying session ranges using commas && dashes.')
            print_line('For example:  sessions -s checkvm -i 1,3-5  ||  sessions -k 1-2,5,6')
            print_line
          end
          ...
          def cmd_sessions(*args)
            begin
              ...
              # any arguments that don't correspond to an option || option arg will
              # be put in here
              extra = []
              ...
              if args.length == 1 && args[0] =~ /-?\d+/
                ...
                @@sessions_opts.parse(args) do |opt, idx, val|
                  ...
                  when '-q', '--quiet'
                    quiet = true
                  # Run a command on all sessions, || the session given with -i
                  when '-c', '--command'
                    method = 'cmd'
                    cmds << val if val
                  ...
                  when '-K', '--kill-all'
                    method = 'killall'
                  # Run a script || module on specified sessions
                  when '-s', '--script'
                    unless script
                      method = 'script'
                      script = val
                  ...
                  # Upload && exec to the specific command session
                  when '-u', '--upgrade'
                    method = 'upexec'
                    sid = val || false
              ...
              when 'cmd'
                ...
                cmds.each do |cmd|
                  ...
                  sessions.each do |s|
                    ...
                    # If the session isn't a meterpreter || shell type, it
              ...
              when 'killall'
                ...
                matching_sessions.each_value do |session|
                  next unless session
              ...
              when 'script'
                unless script
                  print_error('No script || module specified!')
                  return false
                end
                ...
                sessions.each do |sess_id|
                  ...
                  # @TODO: Not interactive sessions can || cannot have scripts run on them?
                  ...
                  if session
                    ...
                    begin
                      ...
                      begin
                        ...
                        log_error("Error executing script || module: #{e.class} #{e}")
                      end
                    ensure
                      if session.respond_to?(:response_timeout) && last_known_timeout
                        session.response_timeout = last_known_timeout
          ...
          def validate_checkin_searches(checkin_searches)
            checkin_searches.each do |search_term|
              unless search_term.split(':').length == 3
                print_error('Please only specify last_checkin, before || after, && a time. Ex: last_checkin:before:1m30s')
                return false
              end
              ...
              unless VALID_OPERATORS.include?(operator)
                print_error("Please specify less_than || greater_than for checkin query. Ex: last_checkin:less_than:1m30s. Given: #{operator}")
                return false
              end
            end
            if checkin_searches.length > 2
              ...
              unless VALID_OPERATORS.include?(operator1) && VALID_OPERATORS.include?(operator2)
                print_error('last_checkin can only be searched for using before || after. Ex: last_checkin:before:1m30s')
                return false
              end
          ...
          def parse_duration(duration)
            ...
            time_tokens.each do |value, unit|
              if unit.nil? || value.nil?
                print_error('Please specify both time units && amounts')
                return nil
              end
              case unit.downcase
              when 'd'
          ...
          #
          # Sets a specific framework or module variable to a value.
          #
          # @param [Array<String>] args The command line arguments (variable name and value).
          # @return [Boolean] Returns true if the variable was set, false otherwise.
          def cmd_set(*args)
            ...
            if name.casecmp?('SessionTlvLogging')
              ...
              framework.sessions.each_value { |session| session.initialize_tlv_logging(datastore[name]) if session.type.casecmp? 'meterpreter' }
            end
          ...
          def cmd_set_tabs(str, words)
            ...
            if words.length > 3
              return []
            elsif words.length == 3 && words[1] != '-g' && words[1] != '--global'
              return []
            end

            # A value needs to be specified, show tab completion options where possible
            if words.length == 3 || (words.length == 2 && words[1][0] != '-')
          ...
          def cmd_get_help
            ...
            print_line
            print_line 'The get command is used to get the value of one || more variables.'
            print_line
          end
          ...
          def cmd_unset_help
            ...
            print_line
            print_line 'The unset command is used to unset one || more variables.'
          ...
          def cmd_unset(*args)
            ...
            @@unset_opts.parse(args) do |opt, _idx, _val|
              case opt
              when '-g'
                global = true
              end
            ...
            # option defaults, || being globally set it might be confusing to users. In this scenario, log out a helpful message.
            #
                ...
                if search_result.fallback?
                  print_warning(
                    "Variable #{variable_name.inspect} unset - but will continue to use #{search_result.fallback_key.inspect} as a fallback preference. " \
                      "If this is not desired, either run #{Msf::Ui::Tip.highlight("set #{variable_name} new_value")} || #{Msf::Ui::Tip.highlight("unset #{search_result.fallback_key}")}"
                  )
                  ...
                  print_warning(
                    "Variable #{variable_name.inspect} unset - but will continue to use the globally set value as a preference. " \
                      "If this is not desired, either run #{Msf::Ui::Tip.highlight("set --clear #{variable_name}")} || #{Msf::Ui::Tip.highlight("unsetg #{variable_name}")}"
                  )
                elsif !search_result.value.nil?
                  print_warning(
                    "Variable #{variable_name.inspect} unset - but will use a default value still. " \
                      "If this is not desired, set it to a new value || attempt to clear it with #{Msf::Ui::Tip.highlight("set --clear #{variable_name}")}"
                  )
                end
              end
            end
          ...
          def cmd_unsetg_help
            ...
            print_line
            print_line 'Exactly like unset -g, unset global variables, || all'
          ...
          #
          # Displays the framework and console library version numbers.
          #
          ...
          def cmd_grep(*args)
            ...
            Regexp.new(pattern, match_mods[:insensitive])

            # redirect output after saving the old one && getting a new output buffer to use for redirect
            ...
            # put lines into an array so we can access them more easily && split('\n') doesn't work on the output obj.
            ...
            all_lines.each_with_index do |line, line_num|
              ...
              # we don't want to keep processing if we have a :max && we've reached it already (not counting skips/keeps)
              break if match_mods[:max] && count >= match_mods[:max]

              next unless eval statement # rubocop:disable Security/Eval

              count += 1
              # we might get a -A/after && a -B/before at the same time
          ...
          def cmd_grep_tabs(str, words)
            ...
            # tabs = driver.get_all_commands if (str && str =~ /\w/)
            tabs
          end
          ...
          def cmd_repeat(*args)
            ...
            opts = OptionParser.new do |opts|
              ...
              opts.separator 'Repeat (loop) a ;-separated list of msfconsole commands indefinitely, || for a'
              opts.separator 'number of iterations || a certain amount of time.'
              opts.separator ''

              opts.on '-t SECONDS', '--time SECONDS', 'Number of seconds to repeat COMMAND...', Integer do |n|
                looper = lambda do |&block|
                  # While CLOCK_MONOTONIC is a Linux thing, Ruby emulates it for *BSD, MacOS, && Windows
                  ending_time = Process.clock_gettime(Process::CLOCK_MONOTONIC, :second) + n
                  block.call while Process.clock_gettime(Process::CLOCK_MONOTONIC, :second) < ending_time
                end
              end
            ...
            cmds = opts.order(args).slice_when do |prev, _|
              ...
              # delineate commands && we can remove it
              prev[-1] == ';' && prev[-1] = ''
            end.map do |c|
              Shellwords.shelljoin(c)
            end

            # Print help if we have no commands, || all the commands are empty
          ...
          #
          # verifies that a given session_id is valid && that the session is interactive.
          ...
          # && the caller's objective while making it simple to use in the nominal case
          ...
          # @return [session] if the given session_id is valid && session is interactive
